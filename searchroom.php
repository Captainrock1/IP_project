...
<html>
  ...
  <body class="bodyimg">
    <ul>
      <li><a href="welcome.php"><i class="fa-solid fa-hotel">&nbsp;&nbsp;Next-Gen Hotel</i></a></li>
    ...
    <div class="gridspace">
        ...
        <div class="left-grid">
          ...
          <form action="searchroom.php" method="get" enctype="multipart/form-data" class="boxcontent2" align="center">
            ...
            <a href="index.php" class="button button1"><i class="fa fa-reset" aria-hidden="true">Cancel</i></a>
          </form>
        </div>
        <div class="mid-grid">
          <div class="boxcontent" align="center">
            <h2 class="headcolor">Rooms</h2>
          </div>
         <?php
            if(isset($_GET['search'])){

              $search_floor = $_REQUEST['floor'];
              $search_roomnum = $_REQUEST['roomnum'];
              $search_roomtype = $_REQUEST['roomtype'];

          }
            if($search_floor == "0"){
                echo "<script>alert('โปรดใส่สิ่งที่ต้องการค้นหา');</script>";
          }
          else if($search_roomnum == "0"){
            echo "<script>alert('โปรดใส่สิ่งที่ต้องการค้นหา');</script>";
            }
          else  if($search_roomtype == "0"){
                echo "<script>alert('โปรดใส่สิ่งที่ต้องการค้นหา');</script>";
            }
          else{
            $search_query = "SELECT * FROM rooms WHERE room_number LIKE'$search_roomnum' AND room_type LIKE'$search_roomtype' AND room_floor LIKE'$search_floor' ";
            $run_query = mysqli_query($db, $search_query);
        ...
        ?>
          <div class="boxcontent2">
              ...
              ?>
                <br>
                <br>
                <input type="submit" value="Can't Booking" class="cantg cant">
              <?php
                }
              ?>
              <?php
              ...
              ?>
                <br>
                <br>
                <a href="booking.php?id=<?php echo $room_id?>"><input type="submit" value="Booking" class="buttoncang buttoncan"></a>
              <?php
                }
              ?>
              </div>
